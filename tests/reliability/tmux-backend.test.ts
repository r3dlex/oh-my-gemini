...
describe('reliability: tmux runtime backend', () => {
  ...
  test('startTeam falls back to process env OMG_STATE_ROOT when input env is omitted', async () => {
    ...
    const previousOmpTeamStateRoot = process.env.OMG_TEAM_STATE_ROOT;
    const previousOmxTeamStateRoot = process.env.OMX_TEAM_STATE_ROOT;
    const previousOmpStateRoot = process.env.OMG_STATE_ROOT;
    ...
    try {
      ...
      if (previousOmpTeamStateRoot === undefined) {
        ...
        process.env.OMG_TEAM_STATE_ROOT = previousOmpTeamStateRoot;
      }
      ...
      if (previousOmpStateRoot === undefined) {
        ...
        process.env.OMG_STATE_ROOT = previousOmpStateRoot;
      }
    }
  });
