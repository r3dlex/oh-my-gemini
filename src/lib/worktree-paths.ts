 ...
 *
 * When Claude Code runs inside a worktree (.gemini/worktrees/X), it encodes the
 * worktree CWD into the project directory path, creating a transcript_path like:
 *   ~/.gemini/projects/-path-to-project--claude-worktrees-X/<session>.jsonl
 *
 * But the actual transcript lives at the original project's path:
 *   ~/.gemini/projects/-path-to-project/<session>.jsonl
 *
 * Claude Code encodes `/` as `-` (dots are preserved). The `.gemini/worktrees/`
...
export function resolveTranscriptPath(transcriptPath: string | undefined, cwd?: string): string | undefined {
  ...
  if (markerIdx !== -1) {
    ...
    if (sessionFile) {
      ...
      const configDir = process.env.CLAUDE_CONFIG_DIR || join(homedir(), '.gemini');
