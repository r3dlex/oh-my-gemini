...
## Purpose
Persistent tracking helpers for subagent lifecycle state and append-only event logs.

## Key Files

| File | Description |
|------|-------------|
| `index.ts` | Implementation for the `subagent-tracker` hook module. |
...
### Internal
- Builds on atomic write and state filesystem helpers.
