...
export async function writeWorkerContext(input: TeamStartInput): Promise<void> {
  ...
  const content = [
    ...
    '- `team` (/team, team run): Multi-worker team orchestration',
    '- `review` (/review): Code and design review',
    '- `verify` (/verify): Completion verification and evidence collection',
    '- `handoff` (/handoff): Task handoff between workers',
    '',
