...
## Purpose
Permission-request screening helpers that classify safe versus review-required commands.

## Key Files

| File | Description |
|------|-------------|
| `index.ts` | Implementation for the `permission-handler` hook module. |
...
### Internal
- Used by the hook pipeline when processing tool permission requests.
