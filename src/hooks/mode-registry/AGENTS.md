...
## Purpose
Execution-mode registration, active-state lookup, and exclusivity helpers.

## Key Files

| File | Description |
|------|-------------|
| `index.ts` | Implementation for the `mode-registry` hook module. |
...
### Internal
- Builds on `src/lib/mode-state-io.ts` and `src/lib/worktree-paths.ts`.
