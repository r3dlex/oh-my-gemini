...
## Purpose
Hook adapter that activates or routes Autopilot mode from prompt keywords and session events.

## Key Files

| File | Description |
|------|-------------|
| `index.ts` | Implementation for the `autopilot` hook module. |
...
### Internal
- `src/modes/autopilot.ts` provides the execution flow.
- `src/hooks/keyword-detector/` and `src/hooks/mode-registry/` drive activation and exclusivity checks.
