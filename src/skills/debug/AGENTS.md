...
## Purpose
Bundled runtime `debug` skill asset used by OMG itself. Investigate a failure, isolate root cause, and propose or apply a fix.

## Key Files

| File | Description |
|------|-------------|
| `SKILL.md` | Bundled runtime skill prompt and metadata for `debug`. |
